...
Résultat attendu : **7 fichiers, 73 tests, tous verts** (aucun appel réseau, aucun coût LLM —
...
| **Garde-fou d'accès** | `tests/unit/guard.test.ts` | `frontend/src/server/auth/guard.ts` | `ADMIN_TOKEN` absent en production → **503 fail-closed** ; absent en dev → accès local ; jeton absent/erroné → 401 ; jeton valide → `ownerId` ; **`isAppPublic()`** (défaut privé, `=1` ouvre) |
