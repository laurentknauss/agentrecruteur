...
function CandidateQAContainer({ locked = true }: { locked?: boolean }) {
  const [isUploading, setIsUploading] = useState(false);
  const [accessAsked, setAccessAsked] = useState(false);
  ...
  return (
    <div className="relative overflow-hidden bg-transparent py-16">
      {/* Popup d'accès restreint : s'ouvre quand un visiteur clique sur la zone d'upload */}
      {locked && accessAsked && (
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 px-4"
          onClick={() => setAccessAsked(false)}
        >
          <div
            role="dialog"
            aria-modal="true"
            ...
            <div className="relative z-20">
              {isUploading ? (
                ...
                <FileUpload
                  ...
                  locked={locked}
                  onLockedClick={() => setAccessAsked(true)}
                />
              )}
